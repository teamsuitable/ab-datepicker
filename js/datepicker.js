	...
	Datepicker.prototype.createDateFromFormat = function(format, value) {
		...
		$.each(format.match(/(.).*?\1*/g), function(k, token) {
			...
			switch (token) {
				...
				case 'LLL':
					month = 0;
					for (var i = 0; i < dateSelf.locales.month_names_abbreviated.length; i++) {
						var month_name = dateSelf.locales.month_names_abbreviated[i];
				...
				case 'LLLL':
					month = 0;
					for (var i = 0; i < dateSelf.locales.month_names.length; i++) {
						var month_name = dateSelf.locales.month_names[i];
						if (value.substring(pos, pos + month_name.length).toLowerCase() == month_name.toLowerCase()) {
							month = i + 1;
							pos += month_name.length;
							break;
				...
				case 'E':
				case 'eee':
					pos += skipName(dateSelf.locales.day_names_abbreviated, pos);
					break;
				case 'EEEE':
				case 'eeee':
				case 'cccc':
					pos += skipName(dateSelf.locales.day_names, pos);
					break;
				case 'EEEEEE':
				case 'eeeeee':
				case 'cccccc':
					pos += skipName(dateSelf.locales.day_names_short, pos);
					break;
				case 'MM':
				case 'M':
				case 'LL':
				...
				case 's':
					...
					break;
				case 'a':
					var amlength = dateSelf.locales.day_periods.am.length;
					var pmlength = dateSelf.locales.day_periods.pm.length;
					if (value.substring(pos, pos + amlength) == dateSelf.locales.day_periods.am) {
						ampm = "AM";
						pos += amlength;
					} else if (value.substring(pos, pos + pmlength) == dateSelf.locales.day_periods.pm) {
						ampm = "PM";
						pos += pmlength;
					} else {
						return null;
